...
import { getAttachedTeams,getActivityTeams } from '../../../handlers/handlers-get';  // расчеты
  ...
  try {
    ...
    switch (req.method) {
      case 'GET':
        ...
        const activityTeams = await getActivityTeams(attachedTeams, activeTimeRepository);
        // отправляем ответ
        res.status(200).json({
          success: true,
