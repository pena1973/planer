...
import { updateLead } from './../../../handlers/handlers-update';  // расчеты
  ...
  try {
    ...
    switch (req.method) {
      ...
      case 'POST':
        ...
        const { leadId, status, notes } = req.body as RequestBody;

        const resLead = await updateLead(locale, leadId, status, notes, leadRepository)
        if (!resLead.success) {
          res.status(200).json({
            success: false,
