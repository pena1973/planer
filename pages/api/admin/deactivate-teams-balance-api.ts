...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  try {
    switch (req.method) {
      case 'GET': {
        ...
        const costByTeam = new Map<number, number>(
          costs.map(c => [c.teamId, round2(c.amountteam ?? 0)])
        );
        ...
        for (const main of mainTeams) {
          ...
          // прогноз по группе = сумма расходов по id
          const forecast = round2(groupTeamIds.reduce(
            (sum, id) => sum + (costByTeam.get(id) ?? 0),
            0
          ));
    ...
    console.error('Ошибка подключения или выполнения запроса (deactivate-teams-balance-api):', error);
