...
export default function PlanScaleContainer({
  ...
  const generateTimeScalePlan = useCallback((calendarItem: CalendarItem) => {
    const dayScale: React.ReactNode[] = [];
    ...
    const intervalWidth = dayWidth / quants;

    const visibleStartPx = Math.max(0, -shift);
    const visibleEndPx = timelineWidth;
    const visibleStartQuant = Math.max(startQuant, Math.floor(visibleStartPx / intervalWidth) + startQuant);
    const visibleEndQuant = Math.min(finishQuant, Math.ceil(visibleEndPx / intervalWidth) + startQuant);

    for (let i = visibleStartQuant; i < visibleEndQuant; i++) {
      const intervTime = i * 5;

      const isWorkTime = intervTime >= calendarItem.timeStartWork && intervTime < calendarItem.timeFinishWork;
      const isBreakTime = calendarItem.breaks.some(b =>
        intervTime >= b.timeStart && intervTime < b.timeFinish
      );
      ...
      const hourStyleFoo = (
        ...
        if (scale < 50 && [0, 4, 8, 12, 16, 20].includes(hours) && minutes === 0)
          return { hourStyle: styles.interval4hours, hoursValue: String(hours), minutesValue: "" };
        if (scale >= 50 && minutes === 0)
          return {
            hourStyle: [0, 4, 8, 12, 16, 20].includes(hours)
              ? styles.interval4hours
              : styles.interval1hour,
            hoursValue: String(hours),
            minutesValue: ""
          };
        if (scale >= 80 && minutes === 30)
          return { hourStyle: styles.interval30min, hoursValue: "", minutesValue: "" };
        if (scale >= 200 && [0, 5, 10, 15, 20, 25, 35, 40, 45, 50, 55].includes(minutes))
          return { hourStyle: styles.interval5min, hoursValue: "", minutesValue: "" };
      ...
      const unitLoadBlockseReactNodesInner = unitsViewInner.current.map(unitView => {
        let unit_unloadEx = "";
        const exs = unitExceptions.filter(ex =>
          ex.unitId === unitView.id &&
          ex.date === calendarItem.date.toLocaleDateString("en-CA")
        );

        if (exs.length > 0) {
          const exNotWork = exs.find(ex =>
            ex.type === TimeTypeEnum.notWork &&
            intervTime >= ex.timeStart &&
            intervTime < ex.timeFinish
          );
          if (exNotWork) unit_unloadEx = styles.nonWorkTime;

          const exWork = exs.find(ex => ex.type === TimeTypeEnum.work);
          if (exWork)
            unit_unloadEx = intervTime >= exWork.timeStart && intervTime < exWork.timeFinish
              ? styles.workTime
              : styles.nonWorkTime;

          const exBreak = exs.filter(ex => ex.type === TimeTypeEnum.breack);
          const isBreakTimeEx = exBreak.some(b =>
            intervTime >= b.timeStart && intervTime < b.timeFinish
          );
        ...
        const dateLoad = unitLoads.filter(lo =>
          lo.unit.id === unitView.id &&
          lo.date === calendarItem.date.toLocaleDateString("en-CA") &&
          intervTime <= lo.timeStart &&
          lo.timeStart < intervTime + 5
        );

        const operBlocksReactNodes = dateLoad.map((load, index) => (
          <LoadInner
            key={index}
            dayWidth={dayWidth}
            quants={quants}
        ...
        return (
          <div
            ...
            className={`${styles.unit_unload} ${unit_unloadEx}`}
          >
      ...
      // Внешние юниты (аналогично внутренним, можешь оставить прежнюю логику если хочешь)

      const unitLoadBlockseReactNodesOuter = unitsViewOuter.current.map(unitView => {
        const dateLoad = unitLoads.filter(lo =>
          lo.unit.id === unitView.id &&
          new Date(lo.date).toDateString() === new Date(calendarItem.date).toDateString() &&
          intervTime <= lo.timeStart &&
          lo.timeStart < intervTime + 5
        );
    ...
    return dayScale;
  }, [dayWidth, timelineWidth, shift, scale, unitLoads, units]);


  // Функция для генерации шкалы времени  и загруза юнитов для одного дня
  const generateTimeScalePlan_old = (calendarItem: CalendarItem) => {
